...
| Feature | Description |
| --- | --- |
| :art: Qwen Image 2.1 (`/qwen_image`) (NEW!) | Generate images with Qwen Image 2.1. Optional Google Translate of the prompt to English and optional prompt enhancement by a Qwen 8B model. Supports prompt, negative prompt, translate, enhance, steps, megapixels, aspect ratio, sampler, scheduler, CFG, lora, lora_strength, and seed. |
...
| :broom: Flush (`/flush`) | Unloads all ComfyUI models and execution cache to free VRAM/RAM. Owner and admins only. |
| :hammer_and_wrench: Admin Panel (`/admin`) | Owner can restart the bot, reload cogs (`/reload`), manage admins, and manage bans. |
...
| `prompt` | Text prompt (required) |
| `negative` | Negative prompt |
| `translate` | Translate the prompt to English with Google Translate (optional, off by default — toggle on to enable) |
...
| `sampler` / `scheduler` | Sampler and scheduler (user or workflow default if none is chosen) |
| `cfg` | Guidance scale |
| `lora` | LoRA name (optional; dynamic autocomplete from `models/loras`). Defaults to the LoRA set in the config `lora` key; type `none` to disable the loader for one run |
| `lora_strength` | LoRA strength (optional, default 1.0) |
| `seed` | Seed (optional) |
| `stealth` | Ephemeral output |

> **Translation & enhancement:** Both toggles are off by default. `translate: true` routes the positive prompt through a Google Translate node (auto-detect → English) before encoding, so non-English prompts work directly — note that the prompt text is sent to Google Translate when translation is on; otherwise the prompt is sent as-is. `enhance: true` rewrites the prompt with the Qwen 8B model (it preserves your intent, wording for visible text, and style); otherwise the enhancer nodes are removed and the prompt goes straight to the encoder. The embed reports both toggles.
...
> **Spectrum:** The T2I model path runs through [Spectrum (Qwen-Image-2.1)](https://github.com/awdqwdasdg/Comfyui-Spectrum-Qwen2.1) — placed after the attention backend and before the KSampler — for faster sampling. It is tuned to limit the airbrushed look of forecast steps (`flex_window: 0.1` keeps a steady every-other-step real cadence, `tail_actual_steps: 6` keeps the final detail pass fully real, `max_consecutive_forecasts: 4`, `blend_weight: 1.0`); outputs can still look slightly softer than full sampling.
...
> **SDXL checkpoint picker:** Choosing `sdxl` uploads your image, then shows an ephemeral picker listing up to 24 checkpoints from ComfyUI's `models/checkpoints` folder (Discord's select limit) plus a Default option that uses the workflow's own checkpoint. Selecting a checkpoint runs the SDXL upscale workflow with that checkpoint; the embed reports the checkpoint actually used. Switching to a different checkpoint frees ComfyUI memory; reusing the same one does not.
...
Unloads all models and execution cache from ComfyUI. Only the bot owner and promoted admins can run it.
...
Owner only. Hot-reloads all cog extensions (generation, LLM, settings, admin commands) without restarting the process, then re-syncs the slash-command tree.
...
| `bot.py` | Shared bot instance: creates the Bot, loads the four cog slash-command extensions via `load_extension()` in `setup_hook` (registering the persistent `GenerationView` and syncing commands once per boot), starts the LLM model refresh loop on `on_ready`, and closes the shared HTTP session on shutdown (`on_close`) |
...
| `comfyui_client.py` | Async HTTP/WS client for ComfyUI (`/prompt`, `/history`, `/view`, `/upload/image`, `/free`); caches the checkpoint, LoRA, and diffusion-model listings (60s TTL) with a force-refresh fallback when validating a user-supplied name |
...
- **`separate`** — two independent serial lanes (`comfyui` and `llm`). Each lane runs one job at a time, but the two lanes run in parallel, so an image generation and a prompt generation may run concurrently (up to 2 jobs, one per resource). The ComfyUI prompt-gen workflow used by `/gen_prompt` is submitted to the `comfyui` lane while the LLM job holds the `llm` lane, so no GPU work ever overlaps image generations or races their memory flushes.
...
| `generations.db` | Stores generation parameters per Discord message ID (for retry/delete); records older than 30 days are pruned at startup |
...
- The NSFW image check uses **EraX-NSFW-V1.0** (a YOLO11 nano detector exported to ONNX) running entirely on CPU via ONNX Runtime, so it uses RAM rather than VRAM and does not contend with ComfyUI. If the model is missing, the bot auto-downloads and exports it on first boot (you can pick nano/small/medium, defaulting to nano). The one-time YOLO→ONNX export needs `ultralytics` (and the ~2 GB `torch` it pulls in), which is deliberately **not** pinned in `requirements.txt` — run `pip install ultralytics` just for that first boot; once `models/erax_nsfw.onnx` exists it is never needed again.
...
- **Restart:** The `/admin` "Restart Bot" button spawns a fresh process from the project root with the same command-line arguments, then shuts the current bot down gracefully (websocket/HTTP cleanup runs) instead of hard-exiting.
