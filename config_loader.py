...
# Queueing: how resource-heavy work (ComfyUI generations + LLM /gen_prompt)
# is scheduled.
#   unified  -> one single serial queue; image generation and prompt
#               generation never run at the same time (1 concurrent job).
#   separate -> two parallel serial lanes: ComfyUI and LLM each run one job
#               at a time, but the two can run concurrently (up to 2 jobs).
queueing:
  mode: unified
...
models:
  ...
  qwen_image:
    t2i:
      ...
      # TextGenerate) is off unless enhance: true is passed explicitly
      ...
      # prompt through untranslated, False = translate to English). Off by
      # default: the bot always writes True (pass through) unless the
      # translate kwarg is explicitly true.
      translate_node: "773"
      ...
      # Off unless enhance: true; otherwise these nodes are removed and the
      # encoder's prompt input is rewired straight to the translate node.
